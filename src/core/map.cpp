...
void Map::RemoveOldKeyframe()
{
    ...
    for (auto feat : frame_to_remove->left_feature) {
        auto mp = feat->map_point.lock();
        if (mp) {
            mp->RemoveObservation(feat);
        }
    }
    for (auto feat : frame_to_remove->right_feature) {
...
void Map::CleanMap()
{
    ...
    auto iter = active_landmarks_.begin();
    while(iter != active_landmarks_.end())
    {
